...
const (
	freeShippingThresholdUSD = int64(50) // orders with item subtotal >= $50 USD get free shipping
)
...
func (cs *checkoutService) prepareOrderItemsAndShippingQuoteFromCart(ctx context.Context, userID, userCurrency string, address *pb.Address) (orderPrep, error) {
	...
	// Apply free shipping when the item subtotal in USD meets the threshold.
	subtotalUSD, err := cs.itemSubtotalUSD(ctx, cartItems)
	if err != nil {
		return out, fmt.Errorf("failed to compute item subtotal: %+v", err)
	}
...
// itemSubtotalUSD sums the USD prices of all items in the cart.
func (cs *checkoutService) itemSubtotalUSD(ctx context.Context, items []*pb.CartItem) (*pb.Money, error) {
	...
	for _, item := range items {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to get product #%q for subtotal: %+v", item.GetProductId(), err)
		}
		itemTotal := money.MultiplySlow(*product.GetPriceUsd(), uint32(item.GetQuantity()))
		subtotal = money.Must(money.Sum(subtotal, itemTotal))
	}
	return &subtotal, nil
}
