...
const (
	freeShippingThresholdUSD = int64(50) // cart subtotal >= $50 USD qualifies for free shipping
)
...
func (fe *frontendServer) viewCartHandler(w http.ResponseWriter, r *http.Request) {
	...
	for i, item := range cart {
		...
		// Accumulate USD subtotal to evaluate free shipping threshold.
		usdItemTotal := money.MultiplySlow(*p.GetPriceUsd(), uint32(item.GetQuantity()))
		subtotalUSD = money.Must(money.Sum(subtotalUSD, usdItemTotal))
	}

	// Override shipping to free when the USD item subtotal meets the threshold.
